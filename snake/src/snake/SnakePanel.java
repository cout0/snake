...
public class SnakePanel extends JPanel implements KeyListener,ActionListener{
    ...
    ImageIcon title = new ImageIcon("pic/title.png"); // 游戏界面的主题
...
//                isFailed = false;
                ...
                isStarted = !isStarted;
        } else if ((KeyCode ==  KeyEvent.VK_UP && direction != "D")||(KeyCode ==  KeyEvent.VK_W && direction != "D"))
                direction = "U";
          else if ((KeyCode ==  KeyEvent.VK_DOWN && direction != "U" )||(KeyCode ==  KeyEvent.VK_S && direction != "U"))
                direction = "D";
          else if ((KeyCode ==  KeyEvent.VK_RIGHT && direction != "L")||(KeyCode ==  KeyEvent.VK_D && direction != "L"))
                direction = "R";
          else if ((KeyCode ==  KeyEvent.VK_LEFT && direction != "R")||(KeyCode ==  KeyEvent.VK_A && direction != "R"))
                direction = "L";
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (direction.equals("R")){
            ...
            if (snakex[0] == foodx && snakey[0] == foody){  // 吃食物
            		try {//吃食物声音的播放
						SimpleAudioPlayer Eat=new SimpleAudioPlayer(SimpleAudioPlayer.msc_Eat);
						Eat.play();
					} catch (Exception e1) {
            ...
            for (int i = 1; i < len; i ++){  // 如果蛇头碰到自己的身体游戏结束
                if (snakex[0] == snakex[i] && snakey[0] == snakey[i]){
                	try {//死亡的声音播放
						SimpleAudioPlayer Die=new SimpleAudioPlayer(SimpleAudioPlayer.msc_Die);
						Die.play();
					} catch (Exception e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
